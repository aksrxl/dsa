...
public static List<int> SubArraySum(int[] arr, int n, int s)
        {
            var list =new List<int>();
                int sum = arr[0]; // assigning sum as ist element
                int start = 0; // setting starting point as 0
                for (int i= 1; i<= arr.Length; i++){ // remember we are starting from 1
                    while(sum > s && start < i-1){ // remove the start element from sum 
